	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( oldpassword.equals("") || newpassword.equals("")  ) { // 패스워드 변경이 없을때.
			...
			if( reulst ) { // 기존 패스워드가 동일하면 
				...
				response.sendRedirect("/movieproject/member/update.jsp?result=3"); return;
			}
		}
		// DB처리 
		boolean result = MemberDao.getMemberDao().update(member);
		if( result ) { response.sendRedirect("/movieproject/member/update.jsp?result=1"); }
		else { response.sendRedirect("/movieproject/member/update.jsp?result=2"); }
