...
function categoryadd() {
	...
	if (cnamej.test(cname)) { // 정규표현식과 같으면
		...
		categorycheck.innerHTML = "한글 2~20길이로 입력해주세요."; pass[0] = false;
	}
...
$(function() {  // 문서 열리면 해당 코드가 실행 
	...
	$("#mtitle").keyup(function() { 	// mid 가 입력될때마다 해당 함수 실행
		...
		if (mtitlej.test(mtitle)) { // 정규표현식과 같으면
			...
			$.ajax({
				...
				success: function(result) { // 통신 성공 했을경우 (  result = 받은 값 변수 )
					if (result == 1) { // 만약에 받은 데이터가 1이면
						titlecheck.innerHTML = "사용중인 영화제목 입니다."; pass[2] = false;
					} else { // 만약에 받은 데이터가 1이 아니면
						titlecheck.innerHTML = "사용가능한 영화제목 입니다."; pass[2] = true;
					}
				}
			});
		} else {
			titlecheck.innerHTML = "한글, 영문 , 숫자 포함 1~20길이로 입력해주세요."; pass[2] = false;
		}
	}); // keyup end 
	...
	$("#mprice").keyup(function() {
		...
		if (mpricej.test(mprice)) {
			mpricecheck.innerHTML = "등록가능한 가격입니다."; pass[1] = true;
		} else {
			mpricecheck.innerHTML = "가격은 최소 1000원부터 등록이 가능합니다."; pass[1] = false;
		}
	});
	//////////////////////////// 카테고리 중복 체크			
...
let pass = [false, false, false, false, false]; // 배열 = [  ] // 유효성 검사
////////////////////////////////////// form 전송
function movieadd() {
