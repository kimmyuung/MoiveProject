...
function categorybtn() {
	...
	$("#categoryinput").html(
		...
		'<button onclick="catecheck()" type="button" class="form-control">카테고리 중복 검사</button>' +
		'<button onclick="categoryadd()" type="button" class="form-control" autocomplete="off">등록</button>' +
		'<button onclick="categorydelete()" type="button" class="form-control">삭제</button>'
	)
...
$(function() {  // 문서 열리면 해당 코드가 실행 
	...
	$("#mtitle").keyup(function() { 	// mid 가 입력될때마다 해당 함수 실행
		...
		if (mtitlej.test(mtitle)) { // 정규표현식과 같으면
			...
			$.ajax({
				...
				success: function(result) { // 통신 성공 했을경우 (  result = 받은 값 변수 )
					if (result == 1) { // 만약에 받은 데이터가 1이면
						titlecheck.innerHTML = "사용중인 영화제목 입니다."; mpass[0] = false;
					} else { // 만약에 받은 데이터가 1이 아니면
						titlecheck.innerHTML = "사용가능한 영화제목 입니다."; mpass[0] = true;
					}
				}
			});
		} else {
			titlecheck.innerHTML = "한글, 영문 , 숫자 포함 1~20길이로 입력해주세요."; mpass[0] = false;
		}
	}); // keyup end 
	...
	$("#mprice").keyup(function() {
		...
		if (mpricej.test(mprice)) {
			mpricecheck.innerHTML = "등록가능한 가격입니다."; mpass[1] = true;
		} else {
			mpricecheck.innerHTML = "가격은 최소 1000원부터 등록이 가능합니다."; mpass[1] = false;
		}
	});
	...
	$("#mruntime").keyup(function() {
		...
		if (mruntimej.test(mruntime)) {
			mtimecheck.innerHTML = "등록 가능한 시간입니다."; mpass[2] = true;
		} else {
			mtimecheck.innerHTML = "등록 불가능한 시간입니다."; mpass[2] = false;
		}
	});
...
let pass = [false]; // 0번 : 카테고리 유효성 
let mpass = [false, false, false]; // 배열 = [  ] // 유효성 검사
