...
function amountincre(i){
	...
	$.ajax({
		...
		success : function ( maxamount ){
			if( jsonarray[i]['totalamount'] >= maxamount ){ alert("재고가 부족합니다. "); return; }
			// 총금액 업데이트 
			let price = parseInt ( (  jsonarray[i]['totalprice'] / jsonarray[i]['totalamount'] ) ); // 제품 하나의 금액
			jsonarray[i]['totalamount']++; // 수량증가 
			jsonarray[i]['totalprice'] = price * jsonarray[i]['totalamount']; // 증가된 수량의 총금액 업데이트
			updatecart();
		}
	});
}
...
function amountdecre(i){
	if( jsonarray[i]["totalamount"] == 1  ){ alert("최소 수량입니다."); return }
	let price = parseInt ( (  jsonarray[i]['totalprice'] / jsonarray[i]['totalamount'] ) ); // 제품 하나의 금액
	jsonarray[i]['totalamount']--; // 수량감소 
	jsonarray[i]['totalprice'] = price * jsonarray[i]['totalamount']; // 증가된 수량의 총금액 업데이트
	updatecart();
}
